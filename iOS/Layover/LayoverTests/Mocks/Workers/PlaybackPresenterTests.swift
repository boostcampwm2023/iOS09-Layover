...
final class PlaybackPresenterTests: XCTestCase {
    ...
    final class PlaybackDisplayLogicSpy: PlaybackDisplayLogic {
        ...
        var seekVideoDidCalled = false
        var seekVideoViewModel: Models.SeekVideo.ViewModel!

        var reportVideoDidCalled = false
        ...
        func seekVideo(viewModel: Layover.PlaybackModels.SeekVideo.ViewModel) {
            seekVideoDidCalled = true
            seekVideoViewModel = viewModel
        }

        func reportVideo() {
            reportVideoDidCalled = true
        }
    }
    ...
    func test_presentTeleportCell을_호출하면_teleportPlaybackCell을_호출하고_올바른_데이터를_전달한다() {
        ...
        // assert
        XCTAssertTrue(spy.teleportPlaybackCellDidCalled, "presentTeleportCell은 teleportPlaybackCell을 호출하지 못했습니다")
    ...
    func test_presentLeavePlaybackView를_호출하면_leavePlaybackView를_호출하고_올바른_데이터를_전달한다() {
        ...
        // assert
        XCTAssertTrue(spy.leavePlaybackViewDidCalled, "presentLeavePlaybackView는 teleportPlaybackCell을 호출하지 못했습니다")
    ...
    func test_presentResetPlaybackCell을_호출하면_resetVideo를_호출하고_올바른_데이터를_전달한다() {
        ...
        // assert
        XCTAssertTrue(spy.resetVideoDidCalled, "presentResetPlaybackCell은 resetVideo를 호출하지 못했습니다")
    ...
    func test_presentConfigureCell을_호출하면_configureDataSource를_호출하고_올바른_데이터를_전달한다() {
        ...
        // assert
        XCTAssertTrue(spy.configureDataSourceDidCalled, "presentConfigureDataSource는 configureDataSource를 호출하지 못했습니다")
    ...
    func test_presentSetSeemoreButton을_호출하면_setSeemoreButton을_호출하고_올바른_데이터를_전달한다_delete() {
        let spy = PlaybackDisplayLogicSpy()
        sut.viewController = spy
        let response: Models.SetSeemoreButton.Response = Models.SetSeemoreButton.Response(buttonType: .delete, indexPathRow: 0)
        // act
        sut.presentSetSeemoreButton(with: response)

        // assert
        XCTAssertTrue(spy.setSeemoreButtonDidCalled, "presentSetSeemoreButton은 setSeemoreButton을 호출하지 못했습니다")
    ...
    func test_presentSetSeemoreButton을_호출하면_setSeemoreButton을_호출하고_올바른_데이터를_전달한다_report() {
        let spy = PlaybackDisplayLogicSpy()
        sut.viewController = spy
        let response: Models.SetSeemoreButton.Response = Models.SetSeemoreButton.Response(buttonType: .report, indexPathRow: 0)
        // act
        sut.presentSetSeemoreButton(with: response)

        // assert
        XCTAssertTrue(spy.setSeemoreButtonDidCalled, "presentSetSeemoreButton은 setSeemoreButton을 호출하지 못했습니다")
        XCTAssertEqual(spy.setSeemoreButtonViewModel.buttonType, .report)
    }
    ...
    func test_presentProfile을_호출하면_routeToProfile을_호출한다() {
        ...
        // assert
        XCTAssertTrue(spy.routeToProfileDidCalled, "presentProfile은 routeToProfile을 호출하지 못했습니다")
    }
    ...
    func test_presentTagPlay을_호출하면_routeToTagPlay을_호출한다() {
        ...
        // assert
        XCTAssertTrue(spy.routeToTagPlayDidCalled, "presentTagPlay는 routeToTagPlay를 호출하지 못했습니다")
    }
    ...
    func test_presentLoadProfileImageAndLocation을_호출하면_setProfileImageAndLocation을_호출한다_nil일_경우_지역으로_이름_모를_곳을_전달한다() {
        ...
        // assert
        XCTAssertTrue(spy.setProfileImageAndLocationDidCalled, "presentLoadProfileImageAndLocation은 setProfileImageAndLocation를 호출하지 못했습니다")
    ...
    func test_presentSeekVideo를_호출하면_seekVideo를_호출한다() {
        let spy = PlaybackDisplayLogicSpy()
        sut.viewController = spy
        let response: Models.SeekVideo.Response = Models.SeekVideo.Response(willMoveLocation: 1, currentCell: Seeds.PlaybackVideo.currentCell)
        // act
        sut.presentSeekVideo(with: response)

        // assert
        XCTAssertTrue(spy.seekVideoDidCalled, "presentSeekVideo를 호출하지 못했습니다.")
        XCTAssertEqual(spy.seekVideoViewModel.willMoveLocation, 1)
        XCTAssertEqual(spy.seekVideoViewModel.currentCell, Seeds.PlaybackVideo.currentCell)
    }

    func test_presentReportVideo를_호출하면_reportVideo를_호출한다() {
        let spy = PlaybackDisplayLogicSpy()
        sut.viewController = spy

        // act
        sut.presentReportVideo()

        // assert
        XCTAssertTrue(spy.reportVideoDidCalled, "presentReportVideo를 호출하지 못했습니다")
    }
}
